// API 설정
const API_CONFIG = {
    WORD_ANALYSIS_API: 'https://api.wordgenetic.com/analyze', // 실제 API 엔드포인트로 변경 필요
    STORY_API: 'http://localhost:3001/api/story'
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    backButton.addEventListener('click', () => {
        showSearch();
        history.pushState({ screen: 'search' }, '', window.location.pathname);
        searchInput.focus();
    });
    ...
    async function handleSearch(word) {
        try {
            ...
            // 서버에서 단어 데이터 가져오기
            const response = await fetch(`http://localhost:3001/api/word/${word.toLowerCase()}`);
            if (!response.ok) {
